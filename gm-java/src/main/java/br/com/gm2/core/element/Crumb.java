...
public class Crumb {
    ...
    public byte[] hash(byte[] b) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        return Hashing.murmur3_32().hashBytes(b).asBytes();
    }
    ...
    public byte[] processSubset(int[] subset, int[] identity) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        if (Arrays.equals(uniqueness, hash(content))) {
            if (inverse) {
                set.flip(0, n);
