 ...
 *
 */
public class QuickSearchStrategy extends AbstractStrategy {

	private int k, n, d;
	private Crumb crumb;

	public QuickSearchStrategy(int[] subset) {
		this.subset = subset;
	}

	public QuickSearchStrategy() {
	}
	...
	public byte[] algorithm(Crumb crumb) {
		if (subset.length == 0) {
			...
			return initSearch(subset, 0, 0, d, 0);
		}
	}

	private byte[] initSearch(int[] subset, int i, int limit, int dp, int dpa) {
		return parentNodeSearch(subset, i, limit, dp, dpa, G(subset, i, dp, k));
	}

	private byte[] parentNodeSearch(int[] subset, int i, int limit, int dp, int dpa, int h) {
		byte[] result = null;
	...
	private byte[] inspect(int[] subset, int i, int limit, int dp, int dpa) {
		byte[] result = null;
		int dc = measure(subset, i, dpa);
		if (dc == d) {
			result = verify(subset, i, limit);
		} else if (dc > d) {
			result = childNodeSearch(subset, i, limit, dp, dpa, result);
		}
		return result;
	}

	private int measure(int[] subset, int i, int dpa) {
		slide(subset, i);
	...
	private byte[] childNodeSearch(int[] subset, int i, int limit, int dp, int dpa, byte[] result) {
		if (i < k - 1) {
			int diff = identity[i] == subset[i] ? 0 : identity[i] - subset[i];
			int loc = diff == 0 ? 0 : diff * diff;
			result = initSearch(subset, i + 1, subset[i] + 1, dp - loc, dpa + loc);
			if (result != null) {
				subset[i] = limit;
			}
		} else {
	...
	private void slide(int[] subset, int i) {
